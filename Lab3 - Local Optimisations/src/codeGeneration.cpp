...
VALUE_TYPE useArray(treeNode* node, VALUE_TYPE array) {
    ...
    if (arrFoundVal != NULL) {  // get pointer to element of array at index
        ...
        element_ptr =
            LLVMBuildInBoundsGEP(currBuilder, base_ptr, &index, 1, tag.c_str());
    } else {
        base_ptr = array;
        base_ptr = LLVMBuildLoad(
            currBuilder, array,
            "_load_ptr_val");  // loads the base pointer from pointer to pointer
        element_ptr = LLVMBuildGEP(
            currBuilder, base_ptr, &index, 1,
            tag.c_str());  // calculates the actual pointer of the offset
    }
