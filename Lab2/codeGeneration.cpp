...
LLVMIntPredicate toIntPred[6] = {LLVMIntEQ,  LLVMIntNE,  LLVMIntSGT,
                                 LLVMIntSLT, LLVMIntSGE, LLVMIntSLE};
LLVMRealPredicate toRealPred[6] = {LLVMRealOEQ, LLVMRealONE, LLVMRealOGT,
                                   LLVMRealOLT, LLVMRealOGE, LLVMRealOLE};

VALUE_TYPE useArray(treeNode* node, VALUE_TYPE array);
VALUE_TYPE loadValueifNeeded(treeNode* node, VALUE_TYPE prev_val);

VALUE_TYPE searchInTable(
    std::string varName,
    std::stack<std::map<std::string, VALUE_TYPE> >& symTable,
    bool stopFirst = false) {
    if (symTable.empty()) return NULL;
...
DATATYPE_TYPE searchInArrTable(
    std::string varName,
    std::stack<std::map<std::string, DATATYPE_TYPE> >& symTable,
    bool stopFirst = false) {
    if (symTable.empty()) return NULL;
...
FUNCTION_TYPE searchInFuncSymTable(
    std::string varName,
    std::stack<std::map<std::string, VALUE_TYPE> >& funcSymTable) {
...
DATATYPE_TYPE stringToLLVMType(std::string typeName, LLVMContextRef c) {
    if (typeName == "INT")
        ...
        return LLVMInt8TypeInContext(c);  // TODO: Check this
...
VALUE_TYPE useArray(treeNode* node, VALUE_TYPE array) {
    ...
    if (arrFoundVal != NULL) {  // get pointer to element of array at index
        ...
        element_ptr =
            LLVMBuildInBoundsGEP(currBuilder, base_ptr, &index, 1, "");
    } else {
        // printf("%s %s\n", "entered in pointer array access", name.c_str());
        base_ptr = array;

        base_ptr = LLVMBuildLoad(
            currBuilder, array,
            "_load_ptr_val");  // loads the base pointer from pointer to pointer
        element_ptr =
            LLVMBuildGEP(currBuilder, base_ptr, &index, 1,
                         "");  // calculates the actual pointer of the offset
    }
...
// 	printf("%s\n", tag.c_str());
// 	LLVMValueRef base_ptr = LLVMBuildStructGEP(currBuilder, id, 0,
// tag.c_str()); 	LLVMValueRef element_ptr = LLVMBuildInBoundsGEP(currBuilder,
// base_ptr, &index, 1, "");
...
VALUE_TYPE loadValueifNeeded(treeNode* node, VALUE_TYPE prev_val) {
    // loads the value if its an id or a dereference, to be used for lhs vs rhs
    // issues
    LLVMBuilderRef currBuilder = builderStack.top();
    ...
    if (node->type == "Ident" && node->children.size() == 0) {
        ...
        } else {  // if an array variable - likely a param to a function without
                  // [] - calculate base ptr and return
            LLVMBuilderRef currBuilder = builderStack.top();
            VALUE_TYPE generated_code = LLVMBuildStructGEP(
                currBuilder, arrFoundVal, 0, "_arr_base_ptr");
            VALUE_TYPE indices = {LLVMConstInt(
                LLVMInt32TypeInContext(contextStack.top()), 0, false)};
            return LLVMBuildGEP(currBuilder, generated_code, &indices, 1,
                                "_arr_first_elt_ptr");
        }
    }
    // load array value
...
VALUE_TYPE codegenBinExp(treeNode* lhs, treeNode* rhs, LLVMOpcode Op,
                         bool logical = false) {
    LLVMBuilderRef currBuilder = builderStack.top();
    ...
    if (LLVMGetTypeKind(LLVMTypeOf(lhsVal)) == LLVMFloatTypeKind &&
        ...
        return LLVMBuildBinOp(currBuilder, ((LLVMOpcode)(Op + 1)), lhsVal,
                              rhsVal, "_flt_op");
    } else if (LLVMGetTypeKind(LLVMTypeOf(lhsVal)) == LLVMPointerTypeKind &&
    ...
    } else if (LLVMGetTypeKind(LLVMTypeOf(rhsVal)) == LLVMPointerTypeKind &&
...
VALUE_TYPE constShift(treeNode* node, ConstNode* cons, LLVMOpcode Op) {
    ...
    VALUE_TYPE var_incr =
        LLVMBuildBinOp(currBuilder, Op, IdentVal, constVal, tag.c_str());

    return LLVMBuildStore(currBuilder, var_incr, Ident);
}

VALUE_TYPE codegenCondExp(treeNode* lhs, treeNode* rhs, int Op,
                          bool logical = false) {
    LLVMBuilderRef currBuilder = builderStack.top();
    ...
    if (LLVMGetTypeKind(LLVMTypeOf(lhsVal)) == LLVMFloatTypeKind &&
        LLVMGetTypeKind(LLVMTypeOf(rhsVal)) == LLVMFloatTypeKind && !logical) {
        return LLVMBuildFCmp(currBuilder, toRealPred[Op], lhsVal, rhsVal,
                             "flt_cmp");
    } else {
        return LLVMBuildICmp(currBuilder, toIntPred[Op], lhsVal, rhsVal,
                             "int_cmp");
    }
}
...
VALUE_TYPE treeNode::codegen() {
    ...
    if (type == "start") {
        ...
        DATATYPE_TYPE funcRetType =
            stringToLLVMType(children[0]->type, currContext);
        FUNCTION_TYPE funcHeader =
            ((FunctionNode*)children[1])->codegen(true, funcRetType);
        ...
        return NULL;
    } else if (type == "RETURN") {  // TODO: Check if you have to load here or
                                    // return as is
...
VALUE_TYPE DeclNode::codegen(bool isGlobalContext) {
    if (!isGlobalContext) {
        ...
        LLVMContextRef currContext = contextStack.top();
        return ((InitDeclNode*)children[1])
            ->codegen(isGlobalContext, stringToLLVMType(type, currContext));
    } else {
        std::string type = children[0]->type;
        LLVMContextRef currContext = contextStack.top();
        return ((InitDeclNode*)children[1])
...
VALUE_TYPE getGlobalDeclaration(LLVMModuleRef mod, DATATYPE_TYPE type,
                                std::string varName) {
...
VALUE_TYPE getTypeZero(DATATYPE_TYPE type, std::string array_name = "",
                       int array_length = 0) {
    LLVMTypeKind type_kind = LLVMGetTypeKind(type);
    if (type_kind == LLVMIntegerTypeKind) {
        return LLVMConstInt(LLVMInt32TypeInContext(contextStack.top()), 0,
                            false);
        ...
        // // DATATYPE_TYPE elem_type = searchInArrTable(array_name,
        // arrSymTable); DATATYPE_TYPE elem_type = LLVMInt32Type(); for (int i =
        // 0; i < array_length; i++) { 	init_vals[i] = getTypeZero(elem_type);
        // }
...
VALUE_TYPE ArrayNode::codegen(bool isGlobalContext, DATATYPE_TYPE type) {
    ...
    if (isGlobalContext) {
        VALUE_TYPE allocate =
            getGlobalDeclaration(mod, arrayType, varName.c_str());
    ...
    BLOCK_TYPE curBlk = LLVMGetInsertBlock(
        currBuilder);  // TODO: Check if this gives the current block
    VALUE_TYPE parentFuncHeader = LLVMGetBasicBlockParent(curBlk);

    // TODO: Look at the third parameter
    VALUE_TYPE allocate = LLVMBuildArrayAlloca(
        currBuilder, arrayType,
        NULL /*This could probably be the function header value*/,
        varName.c_str());
...
VALUE_TYPE PointerNode::codegen(bool isGlobalContext, DATATYPE_TYPE type) {
    ...
    if (childType == "POINTER") {
        ...
        varPlace =
            ((PointerNode*)children[0])->codegen(isGlobalContext, pointerType);
    } else if (childType == "VARIABLE") {
        DATATYPE_TYPE pointerType = LLVMPointerType(type, 0);
        varPlace =
            ((VariableNode*)children[0])->codegen(isGlobalContext, pointerType);
    } else  // FUNCTION
    {
        DATATYPE_TYPE pointerType = LLVMPointerType(type, 0);
        return ((FunctionNode*)children[0])
            ->codegen(isGlobalContext, pointerType);
    }
    ...
    if (foundVal != NULL) {  // its a function argument
        ...
        if (foundInFunc != NULL) {
            ...
            if (children.size() == 0) {
                ...
                return LLVMBuildCall(currBuilder, foundInFunc, temp, 0,
                                     tag.c_str());
            } else {
                // Function takes args
                ...
                for (auto child :
                     children[0]->children) {  // TODO: ADD LOAD RELATED INFO
                                               // HERE AS WELL
                ...
                return LLVMBuildCall(currBuilder, foundInFunc,
                                     codeForIdents.data(), codeForIdents.size(),
                                     "ident_ret");
            }
        } else {
            return NULL;
...
VALUE_TYPE ConstNode::codegen() {
    if (name == "INT") {
        return LLVMConstInt(LLVMInt32TypeInContext(contextStack.top()), ival,
                            false);
    } else if (name == "FLOAT") {
        ...
        LLVMBuilderRef currBuilder = builderStack.top();
        VALUE_TYPE array = LLVMBuildGlobalStringPtr(
            currBuilder, temp_sval.c_str(), "const_string");
...
FUNCTION_TYPE FunctionNode::codegen(bool isGlobalContext, DATATYPE_TYPE type) {
    ...
    std::map<std::string, FUNCTION_TYPE> topmostSymbolTable =
        funcSymTable.top();
    ...
    if (childType == "VOID") {
        ...
        DATATYPE_TYPE retType =
            LLVMFunctionType(type, paramTypeList, fparams.size(), isVariadic);
        VALUE_TYPE funcDecl = LLVMAddFunction(mod, funcName.c_str(), retType);
...
VALUE_TYPE BranchNode::codegen(DATATYPE_TYPE retType) {
    ...
    if (type == "IF") {
        BLOCK_TYPE endif = LLVMAppendBasicBlock(
            parentFuncHeader, "endif");  // basic block for endif
        BLOCK_TYPE then_blk = LLVMAppendBasicBlock(
            parentFuncHeader, "then");  // basic block for then

        LLVMPositionBuilderAtEnd(currBuilder,
                                 then_blk);  // generated code for then
        ((CondBlockNode*)children[1])->codegen(retType, endif);

        LLVMPositionBuilderAtEnd(currBuilder,
                                 curBlk);  // add to the current block
        VALUE_TYPE conditionCode =
            children[0]->codegen();  // generated code for if
        ...
        BLOCK_TYPE then_blk = LLVMAppendBasicBlock(
            parentFuncHeader, "then");  // basic block for then
        BLOCK_TYPE else_blk = LLVMAppendBasicBlock(
            parentFuncHeader, "else");  // basic block for else
        ...
        LLVMPositionBuilderAtEnd(currBuilder,
                                 else_blk);  // generated code for else
        ...
        LLVMPositionBuilderAtEnd(currBuilder,
                                 then_blk);  // generated code for then
        ((CondBlockNode*)children[1])
            ->codegen(retType,
                      else_blk);  // goes back to the if condition again

        LLVMPositionBuilderAtEnd(currBuilder,
                                 else_blk);  // add to the else block
        VALUE_TYPE conditionCode =
            children[0]->codegen();  // generated code for if
        LLVMBuildCondBr(currBuilder, conditionCode, then_blk, endif);
        ...
        LLVMPositionBuilderAtEnd(currBuilder,
                                 else_blk);  // add to the else block
        VALUE_TYPE conditionCode =
            children[1]->codegen();  // generated code for if
        LLVMBuildCondBr(currBuilder, conditionCode, then_blk, endif);
        ...
        BLOCK_TYPE else_blk = LLVMAppendBasicBlock(
            parentFuncHeader, "else");  // basic block for else
        BLOCK_TYPE increment_blk = LLVMAppendBasicBlock(
            parentFuncHeader, "increment");  // basic block for increment
        ...
        LLVMPositionBuilderAtEnd(currBuilder,
                                 then_blk);  // generated code for then
        ((CondBlockNode*)children[3])
            ->codegen(retType,
                      increment_blk);  // goes back to the if condition again
        ...
        LLVMPositionBuilderAtEnd(currBuilder,
                                 increment_blk);  // add to the increment block
...
void callBlockInst(treeNode* t, LLVMBuilderRef currBuilder,
                   DATATYPE_TYPE retType) {
        ...
        if (childType == "RETURN") {
            if (retType == LLVMVoidType())
                ...
                LLVMBuildRet(
                    currBuilder,
                    t->children[i]
                        ->codegen());  // TODO: Update if return format changes
            }
        } else {
            ...
            } else if (childType == "IF" || childType == "ITE" ||
                       childType == "WHILE" || childType == "DO-WHILE" ||
                       childType == "FOR") {
...
VALUE_TYPE FuncBlockNode::codegen(DATATYPE_TYPE retType,
                                  VALUE_TYPE funcHeader) {
    pushSymTables();
...
VALUE_TYPE CondBlockNode::codegen(DATATYPE_TYPE retTypeIfReqd,
                                  BLOCK_TYPE afterDest) {
    pushSymTables();
